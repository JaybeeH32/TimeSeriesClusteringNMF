...
class Line:
    def __init__(self, args):
        ...
        # self.u_i = tf.keras.layers.Input(name='u_i', dtype=tf.int32, shape=[args.batch_size * (args.K + 1)])
        
        self.u_j = tf.placeholder(name='u_j', dtype=tf.int32, shape=[args.batch_size * (args.K + 1)])
        # self.u_j = tf.keras.layers.Input(name='u_j', dtype=tf.int32, shape=[args.batch_size * (args.K + 1)])

        self.label = tf.placeholder(name='label', dtype=tf.float32, shape=[args.batch_size * (args.K + 1)])
        # self.label = tf.keras.layers.Input(name='label', dtype=tf.float32, shape=[args.batch_size * (args.K + 1)])
        ...
        # self.embedding = tf.Variable('target_embedding', [args.num_of_nodes, args.dimension],initializer=tf.random_uniform_initializer(minval=-1., maxval=1.))
